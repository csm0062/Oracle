...
-- 그룹함수를 사용할 때 주의할 점은 SELECT 절에 포함된 모든 컬럼들은 GROUP BY 절에 명시돼야한다.
...
-- SELECT 절에 포함된 컬럼들이 모두 GROUP BY에 명시돼야하는 규칙때문에 데이터가 변질된다.
SELECT SYEAR
	 , MAJOR 
	 , AVG(AVR)
	 FROM STUDENT
...
-- 부서번호, 부서이름, 부서별 최저급여에 해당하는 사원번호, 사원이름 조회
SELECT A.DNO
	 , A.DNAME
	 , A.MIN_SAL
	 , EE.ENO
...
-- COUNT(특정 컬럼명): 특정 칼럼에 대한 모든 행의 개수를 리턴. 지정된 칼럼에 NULL이 있으면 카운팅을 하지 않는다.
-- 부서별 사원수 조회
SELECT DNO
	 , COUNT(*)
	 FROM EMP
...
-- 1-5. AVG: 그룹된 데이터에 대한 평균값을 구하는 함수
-- 전공별 학년별 평균 평점 조회
SELECT AVG(AVR)
	 , SYEAR
	 , MAJOR
