...
-- 1-3. 참조형 변수 선언(테이블의 컬럼)
DECLARE
	-- STUDENT 테이블의 SNO 칼럼 타입과 동일한 타입으로 지정된다.
	STUDENT_NO STUDENT.SNO%TYPE;
	-- STUDENT 테이블의 SNAME 칼럼 타입과 동일한 타입으로 지정된다.
...
-- 교수가 정교수인지 부교수이닞 출력하는 PL/SQL을 작성하세요.
...
BEGIN
	...
	IF PROFESSOR_ROW.ORDERS = '정교수' THEN
		DBMS_OUTPUT.PUT_LINE(PROFESSOR_ROW.ORDERS || ' 입니다.');
	ELSIF PROFESSOR_ROW.ORDERS = '부교수' THEN
		DBMS_OUTPUT.PUT_LINE(PROFESSOR_ROW.ORDERS || ' 입니다.');
	ELSE 
		DBMS_OUTPUT.PUT_LINE(PROFESSOR_ROW.ORDERS || '정교수, 부교수가 아닌 교수는 시간강사입니다.');
	END IF;
END;
...
BEGIN
	...
	ELSE 
		DBMS_OUTPUT.PUT_LINE(PROFESSOR_ROW.ORDERS || '정교수, 부교수가 아닌 교수는 시간강사입니다.');
	END CASE;
-- CASE2: CASE 뒤에 변수를 지정하지 않고 WHEN정레서 조선식으로 비교
	CASE
...
-- 3-3. FOR LOOP
-- 시작 값과 종료 값을 지정해서 시작 값부터 종료 값까지 반복
DECLARE
	NUM NUMBER;
BEGIN
...
BEGIN -- 일해!
	WHILE CNT < 10 LOOP
		SELECT * INTO STUDENT_ROW
			...
			CNT := CNT + 1;	
	END LOOP;
EXCEPTION --BIGIN뒤에 있어야한다. 예외처리가 발생할시(예외처리가 필요 없을 경우 사용X)
	WHEN NO_DATA_FOUND THEN
		DBMS_OUTPUT.PUT_LINE('해당 데이터를 찾지 못했습니다.');
END;
...
--	STUDENTREC.SCORE_ROW.SNO := '915301';
--	STUDENTREC.SCORE_ROW.CNO := '1211';
--	STUDENTREC.SCORE_ROW.RESULT := '100';
...
BEGIN
	LOOP
		...
		STURECARR(IDX).SYEAR := MOD(IDX, 4) + 1; -- 0부터나오는걸 1부터나오게
...
BEGIN 
	LOOP
		...
		STUROWARR(IDX).SYEAR := MOD(IDX, 4) + 1; -- 0부터나오는걸 1부터나오게
...
BEGIN
	...
	-- NEXT(인덱스): 지정한 인덱스의 다음의 인덱스 리턴
	DBMS_OUTPUT.PUT_LINE(STUROWARR.NEXT(1));

	-- EXISTS(인덱스): 지정한 인덱스에 데이터가 존재하는 지 TURE, FALSE로 리턴
	-- TRUE, FALSE로 리턴되기 때문에 출력할 수 없다. (오라클에 BOOLEAN 타입이 없기 떄문에)
...
-- 6. 커서(CURSOR): CURSOR는 쿼리(SELECT)의 결과를 저장해주는 메모리공간
...
-- 6-3. 커서 파라미터: 커서를 선언할 떄 파라미터도 함께 선언해서 고정적인 쿼리의 결과가 아닌
--				   유동적인 쿼리의 결과를 커서에 저장하는기능
--				   커서를 오픈할 때 파라미터로 지정한 데이터 타입의 값을 함께 넘겨준다.
DECLARE
	-- 파라미터가 있는 커서의 선언
	-- 파라미터들은 조회 쿼리의 조건절(WHERE 절)에서 사용해서 유동적인 쿼리를 실행할 수 있게 해준다.
	-- 파라미터는 원시타입을 넣어준다 (크기나 길이지정불가)
	...
	STUREC STU_REC;
BEGIN
	-- 커서를 오픈할 떄 지정된 파라미터 데이터 타입의 값들을 전달한다.
	OPEN STU_CUR(1, 2.7);

	LOOP
		FETCH STU_CUR INTO STUREC;
	
		DBMS_OUTPUT.PUT_LINE('1학년 학생중 평점이 2.7인 학생목록');
		DBMS_OUTPUT.PUT_LINE(STUREC.SNO);
		DBMS_OUTPUT.PUT_LINE(STUREC.SNAME);
		DBMS_OUTPUT.PUT_LINE(STUREC.SYEAR);
		DBMS_OUTPUT.PUT_LINE(STUREC.MAJOR);
	...
	LOOP
		...
		DBMS_OUTPUT.PUT_LINE('3학년 학생중 평점이 1.5인 학생목록');
		DBMS_OUTPUT.PUT_LINE(STUREC.SNO);
		DBMS_OUTPUT.PUT_LINE(STUREC.SNAME);
		DBMS_OUTPUT.PUT_LINE(STUREC.SYEAR);
		DBMS_OUTPUT.PUT_LINE(STUREC.MAJOR);
...
BEGIN
	...
	LOOP
		...
		DBMS_OUTPUT.PUT_LINE(EMPROW.ENO);
		...
		DBMS_OUTPUT.PUT_LINE(EMPROW.DNO);
		DBMS_OUTPUT.PUT_LINE('--------------------------');
