...
-- 컬럼의 타입은 SELECT구문에서 조회해오는 데이터의 타입과 맞춰지기 때문에 바꿀 수 없다.
-- 과목의 과목번호, 과목이름, 담당교수번호, 담당교수이름, 과목별 기말고사의 성적의 평균이 저장된 테이블 생성
...
-- 학생번호, 학생이름, 학생별 기말고사 성적이 평균점수 (소수점 둘째자리), 학생별 기말고사 평균점수의 등급을 가지는
-- 테이블 ST_AVGRES_GRADE를 생성하세요.
CREATE TABLE ST_AVGRES_GRADE(ST_NO, ST_NAME, ST_AVGRESULT, ST_GRADE)
	AS SELECT A.SNO
		 	, A.SNAME
		 	, A.AVG_RESULT
		 	, GR.GRADE
		 ...
		 GROUP BY A.SNO, A.SNAME, GR.GRADE, A.AVG_RESULT;
	
SELECT *
	FROM ST_AVGRES_GRADE
	ORDER BY ST_NO, ST_NAME,ST_GRADE;
