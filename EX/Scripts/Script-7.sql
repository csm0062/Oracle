...
--                생성되는 테이블들. 직접 조작은 불가능 하고 제공되는 뷰테이블로 SELECT 기능만 이용 할 수 있다.
-- C##STUDY 계정이 사용할 수 있는 데이터 사전조회
SELECT *
   FROM DICT;

-- C##STUDY 계정이 사용할 수 있는 성능 관련 데이터 사전
SELECT *
   FROM DICTIONARY
...
-- 관리자(SYS) 계정이 소유하고있는 시스템 고나련 정보들을 담고있는 테이블 목록 조회
SELECT *
   FROM DBA_TABLES;
...
-- 고유 인덱스는 인덱스로 지정된 컬럼의 데이터가 중복되면 안되는 인덱스(PK, UK)
...
--         원천 테이블의 데이터가 변경되면 VIEW테이블의 데이터도 자동으로 변경된다.
-- 단순 VIEW와 복합VIEW로 나뉘게 되고 
-- 단순 VIEW : 원천 테이블이 하나인 VIEW 테이블 . DML(INSERT, UPDATE, DELETE)구문이 사용가능 
-- 복합 VIEW : 여러개의 원천 테이블을 조인해서 생성한 VIEW 테이블. DML(INSERT, UPDATE, DELETE) 구문이 사용불가능. 
-- 주로 통계 데이터나 원천 테이블의 데이터가 수시로 변경될 때 VIEW를 생성해서 데이터를 저장 후 사용한다. 
--과목별 학과별 기말고사 성적의 평균 점수를 저장하는 VIEW 생성(복합VIEW -- 여러가지 원천테이블을 가지고 있음.COURSE, STUDENT등)
...
--체크 옵션에 위배, 4학년의 데이터만 조회 조건으 ㄹ주었기 때문에 3학년은 안들어감
INSERT INTO ST_SENIOR 
VALUES(
   '999995',
   '장길산',
...
--ORACLE 의 ROWNUM : 조회된 데이터 행 번호를 붙여주는 객체 
--급여 최상위 3명 조회 
SELECT ROWNUM 
   , A.ENO 
    , A.ENAME 
...
--잘못된 쿼리 
--정렬이 되기전에 행번호가 붙어버려서 원하는 데이터와 다른 데이터가 나옴.
--정렬된 데이터에 ROWNUM을 붙인다음에 사용해야 제대로 된 데이터 조회가능
    SELECT ROWNUM 
        , ENO 
        , ENAME 
        , SAL 
 ...
 --전공별 기말고사 성적의 평균 점수가 최상위 3인 데이터 조회 (전공, 기말고사 성적의 평균점수 조회)
       ...
       --잘못된 쿼리. 원하는데이터가 안나옴 WHERE절은 GROUP BY 아래 올수 없기때문에
 SELECT ROWNUM 
    , S.SNAME
    , S.MAJOR 
    , AVG(SC.RESULT) 
    ...
    JOIN SCORE SC 
    ON S.SNO = SC.SNO
    --WHERE 절은 GROUP BY 위에 있어야 함.
    WHERE ROWNUM <= 3
    GROUP BY ROWNUM, S.MAJOR, S.SNAME 
...
, A.S_RESULT
   FROM (
            SELECT S.SNAME 
            , S.MAJOR 
             , AVG(SC.RESULT) AS S_RESULT
             FROM STUDENT S
             JOIN SCORE SC 
             ON S.SNO = SC.SNO
             GROUP BY S.MAJOR, S.SNAME
             ORDER BY S_RESULT DESC
             )A
   WHERE ROWNUM <= 3;
    
--ROWNUM 위치에 따라서 결과가 바뀌기 때문에 
--위치를 잘 지정해야되고 ROWNUM을 바로 활용한 SELECT 구문에서는 <, <=로만 비교가 가능하다. > >= 는 안됨. 
---ROWNUM을 조회하는 SELECT 구문을 서브쿼리로 묶어서 사용하면 >, >= 도 비교 가능하다.
SELECT B.*
   FROM (
      SELECT ROWNUM AS RN
      , A.SNAME 
...
VALUES (EMP_CO_ENO_SEQ1.NEXTVAL, '김화영', '개발', 0 , SYSDATE, 3000, 100, 0);
COMMIT;
...
--INCREMENT BY 2 를 줬기때문에 ENO이 3인 값이 추가됨. 
...
--시퀀스의 수정 
--시퀀스는 모든 옵션을 수정할 수 있지만 시작값은 수정하지 못한다. 
