...
-- AS 키워드를 이용해서 별칭을 붙일 수 있는데 AS 키워드는 생략이 가능하다.
...
SELECT DNO
	 , DNAME
	 , LOC
	 , NVL(DIRECTOR, '팀장 없음') AS DIRECTOR
	 FROM DEPT;
...
-- 보너스가 NULL인 사원은 SAL /12 으로 조회도 가능
SELECT ENO
	 , ENAME
	 , SAL
...
-- 6. 데이터 정렬 기준을 정하는 ORDER BY (거의 맨 마지막에 온다.)
-- 6-1. 컬럼 하나만 정렬
-- 오름차순 (ASC)로 정렬할때는 ASC를 생략해도 된다.
SELECT SNO
	 , SNAME
	 , SYEAR
	 , AVR 
...
-- WHERE 절은 FROM이나 JOIN-ON 절 다음에 작성한다.
...
-- 학생중에 평점이 3.0 이하인 학생번호, 학생이름, 전공, 학년, 평점 조희
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 7-2. WHERE 절에서 값의 동일여부를 비교할때는 =, !=을 사용한다.
...
-- 급여 컬럼에 있는 모든 데이터가 문자열로 변환되는 과정이 필요한다
...
-- 7-4. NULL인 값의 비교는 = NULL, != NULL 로 하지 않고 IS NULL, IS NOT NULL이라는 특수한 구문을 이용한다.
...
-- 부서 테이블에서 팀장이 NULL이 아닌 부서의 번호, 부서이름 조회
SELECT DNO
	 , DNAME
	 FROM DEPT
...
SELECT PNO
	 , PNAME
	 FROM PROFESSOR
	 WHERE PNAME LIKE '___'; -- 언더바 만큼의 글자인
...
-- 사원 중에 회계업무를 하면서 급여가 3000 이상인 이름이 세 글자인 사원의 사원번호, 사원이름, 업무, 급여조회
SELECT ENO
	 , ENAME
	 , JOB
	 , SAL
...
-- () 소괄호를 이용해서 조건의 우선순위를 정해준다.
-- 전공이 화학이거나 물리인 학생중 1학년인 학생의 학생번호, 학생이름, 전공, 학년 조회
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 부서번호가 10이거나 30인 사원중 급여가 3000이상인 사원의 사원번호, 사원이름, 급여, 부서 조회
SELECT ENO
	 , ENAME
	 , SAL
	 , DNO
...
-- WHERE (비교할 컬럼) BETWEEN 시작 값 AND 끝 값;
...
-- 평점이 3.0이상 3.7 이하이면서 학년이 2학년이상 4학년 이하인 학생의 학생번호, 학생이름, 평점, 학년, 전공조회
SELECT SNO
	 , SNAME
	 , AVR
	 , SYEAR
...
-- 큰 따옴표("")는 별칭(alias)를 보여줄 때만 사용한다.
-- 업무가 개발이거나 경영인 사원중에 보너스가 700이상인 사원의 사원번호, 사원이름, 업무, 급여, 보너스조회
SELECT ENO AS "사원번호"
	 , ENAME AS "사원이름"
	 , JOB AS "업무"
	 , SAL AS "급여"
