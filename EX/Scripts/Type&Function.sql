...
-- 1-2. 문자 연산 함수
-- CONCAT(데이터1, 데이터2): 데이터1과 데이터 2가 결합된 새로운 데이터 리턴
SELECT CONCAT (SNAME, AVR)
	FROM STUDENT;

-- CONCAT 함수는 매개변수를 두개만 받을 수 있어서 여러개의 데이터나 다른 표현을 넣고 싶을때는 ||와 조합하여 사용한다.
SELECT CONCAT(SNAME,': '|| AVR)
	FROM STUDENT;
...
-- 오라클의 기본 문자셋은 AL32UTF8 => 영어(1BYTE)를 제외한 다른 문자는 3BYTE씩 계산
SELECT DNAME
 	 , LENGTH(DNAME)
 	 , LENGTHB(DNAME)
 	 FROM DEPT;
...
-- DUAL 테이블: 오라클에서 제공하는 가상의 테이블로
--	  	      간단하게 날짜나 연산 또는 결과 값을 보기 위해서 사용
...
SELECT TRIM(LEADING '0' FROM '00012300') -- 앞 0 제거
	 , TRIM(TRAILING'0'FROM '000123000') -- 뒤 0 제거
	 , TRIM(BOTH'0' FROM '000123000')  -- BOTH는 생략가능
	 , TRIM('   0 0 0 1 2 3 0 0 0   ')  --앞뒤공백제거
	 FROM DUAL;
...
SELECT ENO
 	 , SUBSTR(ENAME, 1, LENGTH(ENAME) -1) -- 첫 번째 글자부터 ENAME의 길이 -1인 글자까지
 	 FROM EMP;
 	
-- 1-3. 문자열 치환함수 
-- TRANSELATE
-- 치환될 문자에 포함된 모든 문자를 치환한다.
SELECT TRANSLATE('World of War','Wo','--') --'World of War'에있는 W 와 o 를 '--'로 치환한다. --> 쪼개서 문자 치환
	FROM DUAL;                             --'World of War'에있는 W 와 o 를 '*-'로 치환할경우 W는 *로 o은 -로 치환됌

-- REPLACE
-- 치환될 문자열과 동일한 문자열만 치환한다.
SELECT REPLACE('World of War','Wo','--') --'World of War'에있는 'Wo'(문자열) 를 '--'로 치환한다. --> 통으로 문자열 치환
	FROM DUAL;
-- 일반 과목을 기초 과목으로 변경해서 모든 과목을 검색하세요
-- (ex. 일반화학 -> 기초화학)
...
SELECT ROUND(123.45678, 3) -- 소수점 셋째자리까지 반올림한것 (즉, 4째에서 반올림됌)
	FROM DUAL;

-- 학생의 학생번호, 학생이름, 전공, 학년, 4.5로 치환된 평점을 조회하는데 평점은 소수점 2째자리까지(ROUND 함수 사용)
SELECT SNO
	 , SNAME
	 , MAJOR
	 , SYEAR
...
-- 1-6. 날짜 함수
-- ROUND(날짜, 날짜단위): 지정한 날짜 단위까지의 날짜는 빼오고 나머지 날짜는 초기화.
-- 날짜를 빼올때 절반이상 지났으면 반올림 된다.
...
-- ADD_MONTHS(날짜, 숫자): 날짜에서 숫자만큼의 개월 수를 더한 날짜 리턴
...
-- LAST_DAT(날짜): 날짜를 포함하고 있는 달의 마지막 날짜를 리턴
SELECT LAST_DAY(SYSDATE)
	FROM DUAL;

-- 사원의 사원번호, 사원이름, 입사일, 입사일 + 100일의 날짜, ,입사일 + 10년 날짜 조회
SELECT ENO
	 , ENAME
	 , HDATE
	 , HDATE + 100
...
-- 1-7. 변환함수
-- TO_CHAR(날짜나 숫자, 변환될 문자열의 현식지정자): 매개변수로 받은 날짜나 숫자 데이터를 지정된 형식으로 변환한 문자열을 리턴
-- 숫자를 문자열로 변환
...
-- 매개변수로 지정된 날짜 형식으로 되어있는 문자열을 DATE타입으로 변환 후 리턴
...
-- TO_NUMBER(문자열 데이터, 문자열 데이터의 숫자 형식)
-- 매개변수로 지정된 숫자 형식으로 된 문자열을 NUMBER 타입으로 변환 후 리턴
...
-- 담당교수가 배정되지 않은 과목과 과목을 배정받지 못한 교수의 정보도 함께 조회
-- 담당교수가 NULL인 데이터는 교수가 배정되지 않음 이라고 조회하고
-- 과목이 없는 교수의 과목정보는 과목 배정받지 못함 이라고 조회
SELECT NVL(C.CNO, '과목 배정 받지 못함')
	 , NVL(C.CNAME, '교수 배정 되지 않음')
	 , NVL(P.PNO, '과목 배정 받지 못함') 
	 , NVL(P.PNAME, '교수 배정 받지 못함')
	 FROM COURSE C
	 FULL OUTER JOIN PROFESSOR P
	   ON C.CNO = P.PNO;
	  
-- 1-9. DECODE(컬럼명이나 조회해온 데이터, 조건 값1,
--	   		   컬럼의 데이너타 조회해온 데이터가 조건 값1과 일치할 때 실행 될 내용,	  
...
--DECODE(컬럼명이나 조회해온 데이터,
--		 조건 값1,
--	   	 컬럼의 데이너타 조회해온 데이터가 조건 값1과 일치할 때 실행 될 내용,	 
--		 조건 값2 
--	   	 컬럼의 데이너타 조회해온 데이터가 조건 값2과 일치할 때 실행 될 내용,
--		 조건 값3,
--	   	 컬럼의 데이너타 조회해온 데이터가 조건 값3과 일치할 때 실행 될 내용,
--		 ...
--		 조건 값N,
--	   	 컬럼의 데이너타 조회해온 데이터가 조건 값N과 일치할 때 실행 될 내용,
--		 컬럼의 데이터나 조회해온 데이터가 조건 값1과 일치하지 않을 실행될 내용(위의 조건과 모두 일치하지 않는 경우)
SELECT ENO
	 , ENAME
	 , DNO
	 , DECODE(DNO,
...
-- 업무가 개발이면 50% 인상, 업무가 경영이면 30% 인상, 업무가 지원이면 20% 인상, 그외 업무 10%이상 된 인상 급여조회
SELECT ENO
	 , ENAME
	 , SAL
	 , DECODE(JOB ,
...
-- CASE (컬럼명)
--	  WHEN 조건1
-- 	  THEN 조건1이 TURE일 때 처리할 내용
--	  WHEN 조건2
-- 	  THEN 조건2가 TURE일 때 처리할 내용	
--    ...
--	  WHEN 조건N
-- 	  THEN 조건N이 TURE일 때 처리할 내용
-- 	  ELSE
--	  위 조건이 모두 FALSE인 데이터에 대한 처리내용
--	  END AS 별칭
-- CASE절에 컬럼이나 데이터를 지정한 경우(SWITCH CASE 문 처럼 사용)
SELECT ENO
	 , ENAME
	 , COMM
	 , CASE NVL(COMM,-1)
...
-- CASE 절에 컬럼이나 데이터를 지정하지 않은경우 (WHEN 절에 비교문을 사용하여 조건을 지정할 수 있다.)
SELECT ENO
	 , ENAME
	 , COMM
	 , CASE
