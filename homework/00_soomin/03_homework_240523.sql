...
SELECT PROFESSOR.* 
	 FROM PROFESSOR
	 WHERE HIREDATE BETWEEN TO_DATE('19910101','YYYYMMDD') 
...
SELECT ENAME
	 , HDATE
	 , TRUNC(MONTHS_BETWEEN(SYSDATE,HDATE)/12) AS YEAR
	 , TRUNC(MOD(MONTHS_BETWEEN(SYSDATE,HDATE),12)) AS MONTH
	 , TRUNC(SYSDATE - ADD_MONTHS(HDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HDATE)))) AS DAY
	FROM EMP;
